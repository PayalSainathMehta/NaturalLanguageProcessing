...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  window_size = skip_window * 2
  window_size = window_size + 1

  #store the data in a data buffer whose length is the same as window's size and gradually move the window and update the buffer i.e. data buffer will act as the window
  #fill the data buffer with the data of the first window
  #The data_index is currently set to 0
  #Moving the data window and updating the data buffer
  ...
  for sample in range(0,n):
      buffer_data = []
      flag = 0
      # print ( data[data_index:data_index+batch_size])
      for i in range(window_size):
          buffer_data.append(data[data_index])
          data_index = data_index + 1
          # print(data_index)
      for balance in range(0,num_skips+1):
          if balance == skip_window:
              flag=1
              continue

          index = sample * num_skips + balance - flag
          batch[index] = buffer_data[skip_window]
          #print("Batch",batch)
          #print (buffer_data[skip_window])
          
          labels[index,0] = buffer_data[balance]
          #print("Labels",label)
          #print (buffer_data[index_of_context_word])
          #list.append(index_of_context_word)
...
if __name__ == '__main__':
  ...
  #valid_examples = np.append(valid_examples, [dictionary["first"], dictionary["american"], dictionary["would"]])
